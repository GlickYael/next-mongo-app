import { MongoClient, ObjectId } from "mongodb";

let client: MongoClient;
let clientPromise: Promise<MongoClient>;

export async function connectDatabase() {
    if (!client) {
        const dbConnectionString = process.env.PUBLIC_DB_CONNECTION;
        if (!dbConnectionString) {
            throw new Error('Database connection string is not defined');
        }
        client = new MongoClient(dbConnectionString);
        clientPromise = client.connect();
    }
    return clientPromise;
}

export async function insertDocument(client: any, collection: string, document: object) {
    const db = client.db('BH-Store');
    console.log(document);

    const result = await db.collection(collection).insertOne(document);
    return result;
}

export async function getAllDocuments(client: any, collection: string) {
    const db = client.db('BH-Store');
    const documents = await db.collection(collection).find().toArray();
    return documents;
}

export async function getDocumentById(client: any, collection: string, id: ObjectId) {
    const db = client.db('BH-Store');
    const document = await db.collection(collection).findOne({ _id: id });    
    return document;
}

export async function updateDocument(client: any, collection: string, document: object, id: ObjectId) {
    const db = client.db('BH-Store');
    const result = await db.collection(collection).updateOne({ _id: id }, { $set: document });
    return result;
}

export async function deleteDocument(client: any, collection: string, id: ObjectId) {
    const db = client.db('BH-Store');
    const result = await db.collection(collection).deleteOne({ _id: id });
    return result;
}


